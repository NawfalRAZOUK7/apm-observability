...
La migration \texttt{0009\_retention\_compression} applique une politique de
rétention (\texttt{APM\_RETENTION\_DAYS} = 90 jours) sur l'hypertable brute, les
vues continues conservant l'historique agrégé de long terme. La compression
colonnaire native de TimescaleDB~\cite{timescaledb-compression} n'est
\emph{pas} activée : elle est incompatible avec la sécurité au niveau ligne
(Row-Level Security) introduite ultérieurement pour le multi-locataire,
l'isolation des locataires primant sur le gain de stockage.

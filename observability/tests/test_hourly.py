# observability/tests/test_hourly.py
from __future__ import annotations

from datetime import timedelta
from typing import Any, Dict, List, Optional, Tuple

from django.db import connection
from django.utils import timezone
from rest_framework import status
from rest_framework.test import APITestCase

from observability.models import ApiRequest


class HourlyEndpointTests(APITestCase):
    URL = "/api/requests/hourly/"

    # --- Helpers -------------------------------------------------

    def _as_rows(self, data: Any) -> List[Dict[str, Any]]:
        """
        Accept both response shapes:
          - list of rows
          - {"source": "...", "results": [...]}
        """
        if isinstance(data, dict) and isinstance(data.get("results"), list):
            return data["results"]
        if isinstance(data, list):
            return data
        self.fail(f"Unexpected response shape: {type(data)} => {data}")

    def _to_regclass(self, name: str) -> Optional[str]:
        with connection.cursor() as cur:
            cur.execute("SELECT to_regclass(%s);", [name])
            row = cur.fetchone()
            return row[0] if row else None

    def _find_hourly_relation(self) -> Optional[str]:
        """
        Try common names for the hourly continuous aggregate / view.
        Return the regclass name if found, else None.
        """
        candidates = [
            "apirequest_hourly",
            "public.apirequest_hourly",
            "observability_apirequest_hourly",
            "public.observability_apirequest_hourly",
        ]
        for c in candidates:
            found = self._to_regclass(c)
            if found:
                return found
        return None

    def _call_with_first_working_timerange(
        self, start_iso: str, end_iso: str
    ):
        """
        Try common param name pairs for time range.
        If endpoint ignores time params, we still accept 200 and validate totals.
        """
        candidates: List[Tuple[str, str]] = [
            ("start", "end"),
            ("time_min", "time_max"),
            ("from", "to"),
            ("time_after", "time_before"),
            ("time__gte", "time__lte"),
        ]

        last = None
        for a, b in candidates:
            res = self.client.get(self.URL, {a: start_iso, b: end_iso})
            last = res
            if res.status_code in (status.HTTP_200_OK, status.HTTP_503_SERVICE_UNAVAILABLE):
                return res

        # If all failed (e.g. 400 validation), return last so caller can show details
        return last

    # --- Setup / Skips ------------------------------------------

    def setUp(self):
        super().setUp()

        # Skip if not PostgreSQL
        if connection.vendor != "postgresql":
            self.skipTest("Hourly endpoint tests require PostgreSQL/TimescaleDB (cagg/view).")

        # Skip if the hourly cagg/view does not exist
        rel = self._find_hourly_relation()
        if not rel:
            self.skipTest("Hourly continuous aggregate/view is missing (apirequest_hourly).")

        # Seed data across multiple hours (single service+endpoint to simplify expectations)
        now = timezone.now()
        base_hour = now.replace(minute=0, second=0, microsecond=0)

        # 3 hourly buckets: H-2, H-1, H
        rows = [
            ApiRequest(
                time=base_hour - timedelta(hours=2, minutes=10),
                service="svc",
                endpoint="/hourly",
                method="GET",
                status_code=200,
                latency_ms=10,
                tags={},
            ),
            ApiRequest(
                time=base_hour - timedelta(hours=2, minutes=5),
                service="svc",
                endpoint="/hourly",
                method="GET",
                status_code=500,
                latency_ms=20,
                tags={},
            ),
            ApiRequest(
                time=base_hour - timedelta(hours=1, minutes=15),
                service="svc",
                endpoint="/hourly",
                method="GET",
                status_code=200,
                latency_ms=30,
                tags={},
            ),
            ApiRequest(
                time=base_hour - timedelta(hours=1, minutes=1),
                service="svc",
                endpoint="/hourly",
                method="GET",
                status_code=200,
                latency_ms=40,
                tags={},
            ),
            ApiRequest(
                time=base_hour + timedelta(minutes=5),
                service="svc",
                endpoint="/hourly",
                method="GET",
                status_code=502,
                latency_ms=50,
                tags={},
            ),
            ApiRequest(
                time=base_hour + timedelta(minutes=10),
                service="svc",
                endpoint="/hourly",
                method="GET",
                status_code=200,
                latency_ms=60,
                tags={},
            ),
        ]
        ApiRequest.objects.bulk_create(rows)

        self.expected_total_hits = len(rows)

    # --- Tests ----------------------------------------------------

    def test_hourly_endpoint_returns_buckets_and_totals(self):
        start = (timezone.now() - timedelta(hours=6)).isoformat()
        end = (timezone.now() + timedelta(hours=1)).isoformat()

        res = self._call_with_first_working_timerange(start, end)

        # If view disappeared / not ready, some implementations return 503
        if res.status_code == status.HTTP_503_SERVICE_UNAVAILABLE:
            self.skipTest(f"Hourly endpoint unavailable (likely missing cagg/view): {res.data}")

        self.assertEqual(res.status_code, status.HTTP_200_OK, res.data)

        rows = self._as_rows(res.data)
        self.assertGreaterEqual(len(rows), 1)

        # Expect presence of key fields
        first = rows[0]
        for key in ("service", "endpoint"):
            self.assertIn(key, first)

        # Detect bucket + hits keys (be tolerant to naming)
        bucket_key = None
        for k in ("bucket", "hour", "time_bucket", "time"):
            if k in first:
                bucket_key = k
                break
        self.assertIsNotNone(bucket_key, f"Missing bucket field in row keys: {sorted(first.keys())}")

        hits_key = None
        for k in ("hits", "count", "requests"):
            if k in first:
                hits_key = k
                break
        self.assertIsNotNone(hits_key, f"Missing hits field in row keys: {sorted(first.keys())}")

        # Totals consistent
        total_hits = sum(int(r.get(hits_key, 0) or 0) for r in rows)
        self.assertEqual(total_hits, self.expected_total_hits)

        # Must have multiple buckets (seeded across 3 hours)
        unique_buckets = {r.get(bucket_key) for r in rows}
        # Some APIs may stringify timestamps; still fine.
        self.assertGreaterEqual(len(unique_buckets), 2, unique_buckets)

        # Ensure our seeded service+endpoint appear
        self.assertTrue(any(r.get("service") == "svc" for r in rows))
        self.assertTrue(any(r.get("endpoint") == "/hourly" for r in rows))

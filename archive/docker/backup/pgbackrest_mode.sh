...
run_pgbackrest() {
  if [ "$LOG_OUTPUT" = "1" ]; then
    ...
    docker compose -f "$SCRIPT_DIR/../docker-compose.backup.yml" exec db /usr/bin/pgbackrest "$@" | tee "$LOGFILE"
    echo "Output logged to $LOGFILE"
  else
    docker compose -f "$SCRIPT_DIR/../docker-compose.backup.yml" exec db /usr/bin/pgbackrest "$@"
  fi
}
